...
const protect = async (req, res, next) => {
    try {
        ...
        // Debug logging to help identify role/decoded token issues during development
        if (process.env.NODE_ENV !== 'production') {
            try {
                console.log('[authMiddleware] decoded token payload:', decoded);
                console.log('[authMiddleware] resolved user id:', user.id, 'role:', user.role);
            } catch (e) {
                console.log('[authMiddleware] debug log error', e.message);
            }
        }
